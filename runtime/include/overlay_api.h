 ...
 * v12: RFE escape callback. Overlay-resident kernel code can contain the real
 *      `rfe` instruction, whose emitted psx_rfe_mark_escape() call must update
 *      the runtime's shared exception state instead of becoming an unresolved
 *      DLL symbol. */
 ...
 *   5: overlay DLL glue forwards emitted psx_rfe_mark_escape() calls through
 *      ABI v12 to the runtime's real exception state. Fresh namespace prevents
 *      stale .ranges coverage from suppressing regeneration after the old DLL
 *      failed to link. */
...
typedef struct {
    ...
    /* RFE exception-escape marker (ABI v12). The recompiler emits this for a
     * guest `rfe`; overlay DLLs must mutate the runtime's shared exception
     * state before their continuation reaches the dispatch trampoline.
     * Appended last so the ABI layout remains monotonic. */
