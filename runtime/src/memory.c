...
int dirty_ram_text_page_diverged(uint32_t phys) {
    ...
    uint32_t page = phys >> DIRTY_RAM_PAGE_SHIFT;
    return (text_diverged_bitmap[page >> 5] >> (page & 31u)) & 1u;
}

/* Evidence-only classification for a game-text PC with no generated dispatch
 * entry. This must not call dirty_ram_text_native_ok(): that API validates a
 * known emitted function range and may mark divergence, while a missing entry
 * has no emitted range to validate. A pristine page has had neither a post-
 * baseline executable DMA load nor a differing CPU write, so its registered
 * reference bytes are still the compile-time EXE bytes. On touched pages the
 * entry instruction is compared only to separate proven runtime code from an
 * ambiguous data/code-page modification; matching one word is intentionally
 * not promoted to proof that an unknown whole function is static. */
DirtyRamTextDispatchClass dirty_ram_text_dispatch_classify(uint32_t phys) {
    if (!text_ref_image || (phys & 3u) != 0u ||
        phys < text_ref_lo || phys >= text_ref_hi ||
        text_ref_hi - phys < sizeof(uint32_t))
        return DIRTY_RAM_TEXT_DISPATCH_UNKNOWN;

    uint32_t page = phys >> DIRTY_RAM_PAGE_SHIFT;
    uint32_t bit = 1u << (page & 31u);
    if (dirty_ram_bitmap[page >> 5] & bit)
        return DIRTY_RAM_TEXT_DISPATCH_RUNTIME;
    if (memcmp(ram + phys, text_ref_image + (phys - text_ref_lo),
               sizeof(uint32_t)) != 0)
        return DIRTY_RAM_TEXT_DISPATCH_RUNTIME;
    if ((text_modified_bitmap[page >> 5] |
         text_diverged_bitmap[page >> 5]) & bit)
        return DIRTY_RAM_TEXT_DISPATCH_MODIFIED;
    return DIRTY_RAM_TEXT_DISPATCH_PRISTINE;
}
