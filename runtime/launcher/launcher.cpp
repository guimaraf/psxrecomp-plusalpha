...
using psx_launcher::kDefaultPadMode;
using psx_launcher::kDigitalDeadzonePct;

static_assert(kDefaultPadMode == PSXRecompV4::PAD_MODE_DIGITAL,
              "launcher default must remain the original PlayStation digital pad");
...
struct LauncherModel {
    ...
    int  p2_dev_index = 0;
    // Pad input mode (PSXRecompV4::PadMode): 0=hybrid, 1=analog, 2=digital
    // (default). Bound to the segmented 3-way selector in each player card.
    int  p1_mode      = kDefaultPadMode;
    int  p2_mode      = kDefaultPadMode;
