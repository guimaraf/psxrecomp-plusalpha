...
bool FunctionAnalyzer::has_callable_boundary(const PS1Executable& exe,
                                             uint32_t addr,
                                             uint32_t max_alignment_nops) {
    const uint32_t exe_lo = exe.header.load_address;
    const uint32_t exe_hi = exe.end_address();
    if ((addr & 3u) != 0 || addr < exe_lo || addr >= exe_hi) {
        return false;
    }

    const auto entry_word = exe.read_word(addr);
    if (!entry_word.has_value() || !is_valid_mips_word(*entry_word)) {
        return false;
    }
    if (addr == exe_lo) {
        return true;
    ...
    for (uint32_t padding_words = 0;
         padding_words <= max_alignment_nops;
         ++padding_words) {
        const uint64_t bytes_back = 8ull + 4ull * padding_words;
        if (static_cast<uint64_t>(addr) <
            static_cast<uint64_t>(exe_lo) + bytes_back) {
            break;
        }

        const uint32_t jr_addr = addr - static_cast<uint32_t>(bytes_back);
        const auto jr_word = exe.read_word(jr_addr);
        const auto delay_word = exe.read_word(jr_addr + 4);
        if (!jr_word.has_value() || !delay_word.has_value() ||
            !is_jr_ra(*jr_word)) {
            continue;
        }

        bool padding_is_nop = true;
        for (uint32_t padding_addr = jr_addr + 8;
             padding_addr < addr;
             padding_addr += 4) {
            const auto padding_word = exe.read_word(padding_addr);
            if (!padding_word.has_value() || *padding_word != 0) {
                padding_is_nop = false;
                break;
            }
        }
        if (padding_is_nop) {
    ...
    int32_t frame = 0;
    if (!is_prologue(*entry_word, frame)) {
        return false;
    }

    if (addr >= exe_lo + 4) {
        const auto previous_word = exe.read_word(addr - 4);
        if (previous_word.has_value() && is_branch_or_jump(*previous_word)) {
            return false;
        }
    }
    return true;
