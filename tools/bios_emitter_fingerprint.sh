...
LC_ALL=C sort <<<"$(printf '%s\n' "${present[@]}")" | while IFS= read -r f; do
    # Force the binary marker (`*`) on every platform. Git Bash already uses
    # binary mode by default, while GNU/Linux normally emits the text marker;
    # an explicit mode keeps the combined digest cross-platform.
    sha256sum --binary "$f"
