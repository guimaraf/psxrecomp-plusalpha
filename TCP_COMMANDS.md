...
| `static_text_misses` | ✓ |   | optional `class`, `min_hits`, `offset`, `limit` | Hot-first external game-text entries without generated code; `class` is `pristine` (default), `modified`, `runtime`, `unknown`, or `all` |
| `overlay_interp_hot` | ✓ |   | optional `sort`, `min_entries`, `phys_lo`, `phys_hi`, `offset`, `limit` | Cumulative hot-first interpreter cost attributed to external overlay-cache-window entries; `sort` is `insns` (default), `entries`, or `hits` |
...
## `static_text_misses` - missing static game coverage (native only)

Detects a missing generated entry before the native-validity gate and classifies
each observation without modifying runtime state. It records only dispatches
that enter the interpreter externally; internal block-to-block interpreter
chaining is deliberately excluded so the bounded table represents actionable
native/interpreter crossings instead of every interpreted branch.

Only a PC on a pristine registered EXE-text page increments `misses`; these rows
are proven static discovery/codegen gaps. The default class is `pristine`.
Diagnostic queries may instead select `modified`, `runtime`, `unknown`, or `all`:

```json
{"id":1,"cmd":"static_text_misses","min_hits":1,"offset":0,"limit":256}
{"id":2,"cmd":"static_text_misses","class":"modified","min_hits":100,"offset":0,"limit":64}
```

The response echoes `class` and contains `total`, `returned`, `offset`, and
`entries`, sorted by descending `selected_hits` and then by PC. Every row has
canonical KSEG0 `pc`, physical `phys`, `selected_hits`, and all four underlying
classification counters (`misses`, `modified`, `runtime`, and `unknown`). The
top-level response adds:

- `observed_total`: all distinct external game-text PCs observed without a
  generated entry;
- `static_observations`: observations proven to be pristine original text;
- `modified_observations`: entry instruction still matches, but its page was
  modified, so the whole unknown function cannot be proven static;
- `runtime_observations`: the page was loaded as runtime executable code or the
  entry instruction differs from the registered EXE;
- `unknown_observations`: the reference/alignment evidence was unavailable.

`modified` remains ambiguous evidence: querying it does not promote those PCs to
static code. Validate the live bytes against the registered EXE and confirm the
function boundary before adding a seed. Consequently, `total=0` is meaningful
only for the selected class and together with all four observation counters.
`dropped` must remain zero; a nonzero value means the bounded evidence table
saturated. The page limit is 256 rows. Query while the game is still running,
before closing its window.

---

## `overlay_interp_hot` - interpreted overlay pressure (native only)

Reports cumulative interpreter work inside the live overlay-cache window without
modifying counters or creating a log file. Rows require `entry_hits > 0`, proving
that native code or the outer dispatcher entered that PC; internal interpreter
block-to-block chaining cannot create a candidate by itself. `insns` includes the
whole locally chained interpreter run attributed to that entry, so it is the
default optimization ranking. `entries` ranks crossing frequency and `hits`
ranks all dispatches to the entry.

```json
{"id":1,"cmd":"overlay_interp_hot","sort":"insns","min_entries":1,"offset":0,"limit":256}
{"id":2,"cmd":"overlay_interp_hot","sort":"entries","phys_lo":"0x001AE000","phys_hi":"0x00200000","offset":0,"limit":64}
```

`phys_lo` is inclusive and `phys_hi` is exclusive; both are intersected with
the runtime's dynamic overlay-cache window. Defaults cover the full 2 MB of PS1
RAM. The response is sorted by descending `selected`, then deterministic
evidence/PC tie-breakers. Every row contains canonical KSEG0 `pc`, physical
`phys`, `selected`, `hits`, `insns`, and `entry_hits`. The top level contains:

- `total`, `offset`, and `returned` for paging the qualifying external entries;
- `window_pcs`, `window_hits`, and `window_insns` across all interpreted PCs in
  the selected dynamic window, including internally chained PCs;
- `external_pcs` and `external_entries` for actionable crossings;
- the effective `sort`, `min_entries`, `phys_lo`, and `phys_hi`.

Counters are cumulative from boot. To isolate one gameplay scenario, pause and
collect every page before the scenario, resume and perform the scenario, pause
again, collect the same pages, then subtract each PC's counters. A positive
`entry_hits` delta proves the entry remained interpreted in that window; rank by
the `insns` delta to select the highest-impact candidate. Do not compare FPS
while issuing the queries: the TCP server is pumped on the main thread, so query
overhead is observer interference rather than gameplay cost.
