...
For a diagnostic micro-batch, restrict the run to one exact captured variant by
its physical/virtual region start and whole-snapshot CRC32:

```sh
python psxrecomp/tools/compile_overlays.py \
    --capture-key 0x001AE000:0x1453A24E \
    <the same arguments shown above>
```

`--capture-key` is repeatable. The region is canonicalized to a physical PSX
address, so `0x801AE000` and `0x001AE000` are equivalent. The command fails if a
requested key is absent; it never silently widens the batch to another variant.
...
| `--capture-key REGION:CRC32` | Compile only the exact captured image identified by region start plus whole-image CRC32. Repeat for multiple explicitly selected variants. Missing keys are an error. |
