...
> **Local-artifact safety.** `bios/`, `psx-bios/`, `generated/`, and all
> `build*` directories are intentionally ignored because they contain private
> inputs or reproducible output. Ignored does not mean disposable: do not run
> `git clean -fdX`, which deletes exactly these local files. Changing
> `.gitignore` never requires removing existing artifacts. If they are lost,
> configure `recompiler/build` again and run `tools/regen_bios.sh` with
> `PSXRECOMP_BIOS_ROM` pointing at your local BIOS.
