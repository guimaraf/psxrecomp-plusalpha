...
- **2026-07-16 (Stage 5.5 clean overlay baseline, no timing-core change):**
  Street Fighter Alpha 2 Gold's same-session A/B snapshots kept physical input
  neutral and every loader integrity counter at zero. The interval added
  293,519,570 dirty-RAM interpreter instructions; `0x801E8000` added
  224,349,734 and `0x801E9D40` added 66,604,492, together 99.1260% of the
  delta. The final local capture key is `0x001AE000:0x542E2B13`. Reinforced-code
  comparison identifies `PL00`, `PL0EX`, the shared player module, and
  `STAGE0E`; the stage matched 5,392/5,392 code bytes, while the initially
  plausible `STAGE06` matched only 454/2,476. Ghidra confirms the two hot PCs
  as independent 152-byte and 1,060-byte functions. The measured 30-second
  profile was 67.21% static, 14.13% interpreter, 0.94% native overlay, and
  17.72% GPU. This is a new coverage baseline for a different fight, not a
  like-for-like timing regression. The next action is an exact-capture shard
  compile and repeat A/B validation by the project owner; no build, shard
  compilation, runtime change, or timing-model change was performed here.

- **2026-07-16 (Digital deadzone guard validated on physical controller):**
  The project owner rebuilt the integrated launcher and confirmed the intended
  policy: fresh/explicit Digital selection uses 40%, while a later manual value
  survives launch and reopen. With `PSX_DEV_INPUT=0`, an untouched Xbox 360
  controller reported `pad=0xFFFF`, `slot0.buttons=0xFFFF`, Digital mode, all
  four stick bytes at 128, `override=-1`, and no valid override axes. A gameplay
  pass then completed with normal controls and no observed gameplay, audio, or
  stability regression. This closes the held-Up frontend regression. No clean
  post-fix `phase_profile` was collected, so no new timing percentages are
  inferred from this validation.

- **2026-07-16 (launcher Digital deadzone guard, no timing-core change):**
  Street Fighter Alpha 2 Gold exposed a frontend-only held-Up regression while
  P1 was Digital with a persisted zero deadzone. TCP `pad_status` proved the
  SIO word was already `0xFFEF`; a neutral override produced `0xFFFF`, a Down
  override produced `0xFFBF`, and restoring a 40% deadzone returned physical
  input to neutral. The launcher now defaults to 40% and resets the shared
  deadzone to 40% whenever either player explicitly selects Digital. A manual
  value chosen after that selection is still persisted and reused until the
  user selects Digital again. Raw-axis-to-percent loading now rounds to the
  nearest percentage so launcher steps (including 40%) survive save/reload
  without losing one point. Fresh launcher sessions are also seeded with the
  same shared 40% policy instead of the previous raw fallback of 12000 (~37%).
  Explicit game or user settings remain authoritative until Digital is selected
  again. Added a standalone structural regression covering the policy and every
  selectable 5% round trip. Guest timing, controller sampling, SIO, code
  generation, and generated game code are unchanged. No build or runtime
  validation was executed; the project owner performs builds and
  hardware-controller validation locally.

- **2026-07-16 (exact overlay micro-batch selection, no timing-core change):**
  Street Fighter Alpha 2 Gold's hot-first telemetry isolated one dynamic image
  at physical region `0x001AE000`, whole-snapshot CRC32 `0x1453A24E`. Its stable
  reinforced-code fingerprint matches the `PAC/STAGE10.PAC` representative.
  Runtime evidence classifies `0x801E8000` as a dispatch entry and
  `0x801E8DF0` as an interior alias owned by the discovered function at
  `0x801E8BB4`; together they account for 99.71% of the measured interpreted
  delta. Added a general, fail-closed `compile_overlays.py --capture-key
  REGION:CRC32` filter so one exact region variant can be compiled and validated
  without widening the experiment to unrelated captures. This does not alter
  guest timing, code generation, the runtime, or the default all-captures path.
  Added a standalone Python regression and user-facing documentation. No build,
  regeneration, shard compilation, or runtime validation was executed; the
  project owner performs builds locally.

- **2026-07-15 (CD-ROM SetLoc consumption and Pause/Read resume position):**
  Street Fighter Alpha 2 Gold's XA sector history captured the first sector
  after Pause at LBA 36638, exactly the stale `08:10:38` SetLoc target, instead
  of continuing from the pre-Pause stream near LBA 36882. The runtime reapplied
  `seek_msf` unconditionally on every ReadN/ReadS and had no state distinguishing
  an unprocessed SetLoc from one already consumed. Added an explicit pending
  SetLoc latch plus a paused-read latch. Read now consumes a pending SetLoc,
  continues an already-active stream without resetting its cursor/FIFO/XA state,
  and, after Pause without a new SetLoc, repeats the most recently received
  sector once as specified by PSX-SPX. Other inactive states retain the current
  cursor rather than being misclassified as Pause. `cdrom_state` and command
  history expose both latches, and a standalone C regression covers all selector
  branches. Snapshot state includes the new latches, and the boot-snapshot
  version is bumped so older layouts are rejected before partial restore. No
  build, regeneration, or runtime validation was executed; the project owner
  performs builds locally.

- **2026-07-15 (static-text telemetry restricted to actionable crossings):**
  Street Fighter Alpha 2 Gold filled 32,755 of 32,768 evidence slots and
  dropped 4,218,820 insert attempts while reporting 802,454,544 modified
  observations. The collector was classifying every interpreted block entry
  before distinguishing external native-to-interpreter dispatches from the
  interpreter's own block-to-block chaining. It now shares the existing
  `g_dirty_interp_chain_target` predicate with `entry_hits` and records only
  external entries. The TCP command accepts `class=pristine|modified|runtime|`
  `unknown|all`, applies `min_hits` and hot-first ordering to the selected
  class, echoes that class, and exposes `selected_hits` while preserving all
  per-PC counters. Modified evidence remains deliberately ambiguous and must
  pass exact original-EXE byte and function-boundary validation before seed
  promotion. Updated the structural regression and TCP documentation. No build,
  regeneration, or runtime validation was executed; the project owner performs
  builds locally.

- **2026-07-15 (static-text miss false negative repaired at the evidence gate):**
  Street Fighter Alpha 2 Gold returned `static_text_misses total=0` even though
  the final dirty-interpreter tail still contained original main-EXE PCs such
  as `0x8011934C` and `0x80126440`. Root cause was a logically unreachable
  classification: generated `psx_game_text_native_ok()` returns false when no
  dispatch entry exists, while the old counter was set only inside its true
  branch. Missing-entry detection now uses the generated non-destructive
  `psx_game_is_function_entry()` predicate before native validation. A new
  memory-side evidence classifier separates pristine registered EXE text,
  modified/ambiguous pages, proven runtime code, and unknown state without
  mutating divergence state. The TCP response pages only proven pristine gaps
  while publishing aggregate and per-PC modified/runtime/unknown counts, so a
  zero static result cannot hide other no-entry execution. Dispatch behavior is
  unchanged: missing or changed text continues through the existing faithful
  live-RAM path. The structural regression now enforces gate ordering and all
  evidence classes. No build, regeneration, or runtime validation was executed;
  the project owner performs builds locally.

- **2026-07-15 (reachable prologue-free callees + static-text miss telemetry):**
  Street Fighter Alpha 2 Gold's fourth-fight sample remained at the normal
  564,480 guest cycles/frame but rose from a median 13,100 to 81,235
  native/interpreter crossings. Exact text validation proved these addresses
  still contain original EXE bytes, not CPU-written overlays. The hottest
  missing Ghidra functions include `0x80156D84`, a real packed B0 BIOS thunk
  reached by a generated `jal` but rejected by the stack-prologue boundary
  heuristic. Exact analysis now reports every reachable direct-JAL target as
  evidence while retaining its conservative root gate; `main_psx` iterates to
  a fixed point, promotes one lowest uncovered target at a time, recomputes
  ownership, and keeps targets already owned by a function as aliases. This
  removes the mid-function
  hard-cap hazard while admitting legitimate leaf/thunk callees. Added unit and
  end-to-end fixtures for a prologue-free B0 thunk. The runtime now counts exact
  original-text dispatch misses separately from DMA/CPU-written code and exposes
  a hot-first, paged `static_text_misses` TCP command; no trace file is written.
  Structural tests for the new telemetry plus the existing CPU-written-overlay
  and RFE-ABI regressions pass. No build, regeneration, or runtime validation
  was executed; the project owner performs those locally.

- **2026-07-15 (CPU-written main-text overlays admitted to capture/cache):**
  Street Fighter Alpha 2 Gold's post-fight transition produced no unknown
  dispatch, stack, or guest-cycle anomaly, but accumulated 457,757,042 dirty
  blocks / 852,325,628 interpreted instructions and up to 153,114 native/interp
  crossings per frame. The dirty-block tail executed live code inside the boot
  EXE range while the final capture omitted those pages. Root cause: ordinary
  CPU stores set the exact text-divergence bitmap, but capture/cache candidacy
  consumed only the CD-DMA dirty bitmap. Unified both proven dynamic-code
  signals for capture, cache candidacy, local interpreter flow, and optional
  JIT admission. Cached DLL entries remain gated by exact live code-range CRC;
  no static generated code or guest behavior was substituted. Added
  `test_overlay_cpu_written_text_capture.py`. No build or runtime validation
  was executed; the project owner performs builds locally.

- **2026-07-15 (overlay-resident RFE link failure fixed at the ABI boundary):**
  Street Fighter Alpha 2 Gold captured kernel-window code containing a real
  `rfe`; both emitters correctly generated `psx_rfe_mark_escape()`, but the
  overlay DLL glue did not export/forward that helper, so GCC rejected the
  affected region and island shards with an undefined reference. Added the
  runtime's real exception-state helper as the final `OverlayCallbacks` member,
  wired it in `overlay_loader.c`, and made `compile_overlays.py` flush pending
  cycles before forwarding the call. Bumped overlay ABI 11->12 and codegen
  namespace cg4->cg5 so stale `.ranges` coverage from the failed cg4 link cannot
  suppress regeneration. Added `test_overlay_rfe_callback.py`; it and the
  existing overlay-init guard pass. No build or runtime validation was executed
  in this session; the project owner performs builds locally.
